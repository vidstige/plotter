...
const fn dup_perm(input: &[u8; 256]) -> [u8; 512] {
    let mut out = [0; 512];
    let mut i = 0;
    while i < 256 {
        out[i] = input[i];
...
const PERM: [u8; 512] = dup_perm(&PERM_SINGLE);
...
pub fn simplex3(p: &Vec3) -> f32 {
    ...
    let gi1 = (PERM
        [ii + i1 as usize + PERM[jj + j1 as usize + PERM[kk + k1 as usize] as usize] as usize]
        % 12) as usize;
    let t1 = 0.6 - x1 * x1 - y1 * y1 - z1 * z1;
    let n1 = corner_contribution(t1, &GRAD3[gi1], &offset1);

    let gi2 = (PERM
        [ii + i2 as usize + PERM[jj + j2 as usize + PERM[kk + k2 as usize] as usize] as usize]
        % 12) as usize;
